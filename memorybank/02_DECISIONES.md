...
| D16 | 2026-09-23 | **Primer backend de servidor: PHP + SQLite**, solo para estadísticas. `api/stats.php` recibe, `admin/index.php` enseña, `api/_bd.php` es la librería común (bloqueada por `.htaccess`, como todo `_*.php`). La base y las credenciales viven en **`~/datos/`**, fuera del docroot y del repo | Matiza D1: la búsqueda sigue siendo 100 % cliente (D11 intacta); el servidor solo registra. PHP es lo que sirve el hosting sin montar nada; SQLite = un fichero, sin usuario ni contraseña de BD, backup copiando `~/datos/ane.db`. El repo es público: nada de datos ni claves dentro |
| D17 | 2026-09-23 | Estadísticas del buscador **anónimas por diseño** y panel `/admin` con **Basic Auth** (usuario + contraseña del navegador, sobre HTTPS) | Lo decide Eduardo. Se guarda término, nº de resultados, cuántos salen solo de la transcripción, página y fecha; y en los clics, vídeo y segundo. Sin IP, sin cookies, sin identificadores: no hace falta banner. Los términos con pinta de correo o teléfono/DNI no se guardan. El hash de la contraseña está en `~/datos/admin.php`; el panel falla cerrado si falta. Documentado en `privacidad.html` |
